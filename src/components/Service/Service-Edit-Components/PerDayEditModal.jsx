...
export default function PerDayEditModal({
  handleClose,
  handleOpen,
  open,
  data,
  ...
  const validationSchema = Yup.object({
    ...
    end_day: Yup.string().when("is_range", ([is_range], schema) => {
      if (is_range === true) {
        return schema.required("End Day is Required");
      } else {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      formiks((prev) => {
        ...
        const findIndex = prev.service_price_service.findIndex(
          (dat) => dat.id === values.id
        );
