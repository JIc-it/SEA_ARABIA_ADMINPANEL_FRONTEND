...
export default function FilterPopup({
  ...
  const handleFilterSubCategory = (e) => {
    ...
    setFilters((prevFilters) => {
      ...
      const subCategoryArray =
        prevFilters.sub_category.length > 0 ? prevFilters.sub_category : [];
  ...
  const handleFilterVendor = (e) => {
    ...
    setFilters((prevFilters) => {
      ...
      const vendorArray =
        prevFilters.vendor.length > 0 ? prevFilters.vendor : [];

      // Check if the value already exists in the vendor array
      const existingVendorIndex = vendorArray.findIndex(
        (item) => item.id === value
      );
                    ...
                    .map((data) => (
                      <div class="form-check">
                        <input
                          ...
                          value={data.id}
                          name={data.name}
                          checked={filters.sub_category.find(
                            (items) => items.id === data.id
                          )}
