...
export default function DiscountAddNew() {
  ...
  const validationSchema = Yup.object({
    ...
    end_date: Yup.string().when("is_lifetime", ([is_lifetime], schema) => {
      if (is_lifetime === false) {
        return schema.required("Validity is Required");
      } else {
        return schema.notRequired();
      }
    }),
    ...
    companies: Yup.array()
      .of(companyObjectSchema)
      .min(1, "Vendor / Service is required"),
  });
    ...
    onSubmit: async (values) => {
      ...
      if (!isLoading) {
        try {
          ...
          const companiesId = values.companies.map((data) => {
            return data.id;
          });
          const servicesId = values.services.map((data) => {
            return data.id;
          });
          ...
          formdata.append("is_lifetime", checktrue(values.is_lifetime));
          {
            values.end_date.trim() !== "" &&
              formdata.append(
                "end_date",
                new Date(values.end_date)?.toISOString().slice(0, -5) + "Z"
              );
          }
  ...
  const CouponCode = (data) => {
    formik.setValues((prev) => {
      return { ...prev, coupon_code: data.toUpperCase() };
    });
  };
  ...
  function companywithservice(companyid) {
    const serviceCount =
      formik?.values.services?.filter((dat) => dat.company_id === companyid) ||
      0;
  ...
  function companywithservicelength(companyid) {
    const serviceCount =
      servicelisting?.filter((dat) => dat.company_id === companyid) || 0;
    return serviceCount.length;
  }
          ...
          <form onSubmit={formik.handleSubmit} style={{marginLeft:"-2%"}}>
            ...
            >
                ...
                <div className={isMobileView ? "w-100" : "w-50"}>
                  <div>
                    ...
                    <input
                      ...
                      name="coupon_code"
                      className="discount-input"
                      onChange={(e) => CouponCode(e.target.value)}
                      onBlur={formik.handleBlur}
                    />
                  ...
                  {formik.values.discount_type === "Percentage" ? (
                    <div className="d-flex" style={{ marginTop: "8px" }}>
                      ...
                      <div>
                        <p style={{ fontWeight: 550, fontSize: "14px" }}>
                          Upto Amount <span style={{ color: "red" }}>*</span>
                        </p>
                        <input
                          type="number"
                          value={formik.values.up_to_amount}
                          name="up_to_amount"
                          className="discount-input"
                          style={{ width: "90%" }}
                          onChange={formik.handleChange}
                          onBlur={formik.handleBlur}
                        />
                        {formik.touched.up_to_amount &&
                        formik.errors.up_to_amount ? (
                          <div className="error">
                            {formik.errors.up_to_amount}
                          </div>
                        ) : null}
                    ...
                    <div style={{ marginTop: "8px" }}>
                      <p style={{ fontWeight: 550, fontSize: "14px" }}>
                        Specify Amount <span style={{ color: "red" }}>*</span>
                      </p>
                      <input
                        type="number"
                        name="discount_value"
                ...
                >
                  <Paper
                    ...
                    style={{
                      ...
                      border:
                        formik.values.redemption_type === "One-Time"
                  ...
                  >
                    ...
                    <Radio
                      name={formik.values.redemption_type}
                      checked={formik.values.redemption_type === "One-Time"}
                    />
                  </Paper>
                  <Paper
                    onClick={() =>
                  ...
                  >
                    ...
                    <Radio
                      name={formik.values.redemption_type}
                      checked={formik.values.redemption_type === "Unlimited"}
                    />
                  </Paper>
                  <Paper
                    onClick={() =>
              ...
              {formik.values.redemption_type === "Limited-Number" && (
                <div style={{ marginTop: "8px" }}>
                  <p style={{ fontWeight: 500, fontSize: "16px" }}>
                    Specify Number <span style={{ color: "red" }}>*</span>
                  </p>
                  <input
                    type="number"
                    value={formik.values.specify_no}
                    name="specify_no"
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    className="discount-input"
              ...
              <div className={isMobileView ? "d-flex flex-column" : "d-flex"}>
                  ...
                  >
                    <Button
                      style={{
                        ...
                        fontSize: "15px",
                        backgroundColor:
                          formik.values.allow_multiple_redeem === "One-Time"
                            ? "black"
                            : "",
                        color:
                          formik.values.allow_multiple_redeem === "One-Time"
                    ...
                    <Button
                      style={{
                        ...
                        textTransform: "capitalize",
                        backgroundColor:
                          formik.values.allow_multiple_redeem ===
                          "Multiple-time"
                            ? "black"
                            : "",
                        color:
                ...
                <div
                  className={isMobileView ? "w-100" : "w-50"}
                  style={{ marginTop: "8px" }}
                >
                  <p style={{ fontWeight: 550, fontSize: "14px" }}>
                    Specify Number{" "}
                  ...
                  <input
                    type="number"
                    name="multiple_redeem_specify_no"
                    disabled={
                      formik.values.allow_multiple_redeem === "One-Time"
                    }
                    value={formik.values.multiple_redeem_specify_no}
                    className="discount-input"
                    style={{ padding: "5px" }}
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                  />
                  {formik.touched.multiple_redeem_specify_no &&
                  formik.errors.multiple_redeem_specify_no ? (
                    <div className="error">
                      {formik.errors.multiple_redeem_specify_no}
                    </div>
                  ) : null}
              ...
              >
                ...
                <input
                  type="datetime-local"
                  value={convertAndFormatDateTime(formik?.values?.start_date)}
                  name="start_date"
                  className="discount-input"
                  style={{ padding: "5px" }}
                  onChange={(e)=>{
              ...
              <div className={isMobileView ? "d-flex flex-column" : "d-flex"}>
                ...
                >
                  ...
                  <input
                    type="datetime-local"
                    value={convertAndFormatDateTime(formik.values?.end_date)}
                    name="end_date"
            ...
            >
              ...
              <div className="d-flex justify-content-between align-items-center">
                <p style={{ fontWeight: 550, fontSize: "14px" }}>
                  Services/Vendors <span style={{ color: "red" }}>*</span>
                </p>
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  <div style={{ display: "flex" }}>
                    <label class="switch" style={{ marginRight: "5px" }}>
                      <input
                        type="checkbox"
                        defaultChecked
                        name="allow_global_redeem"
                        value={formik.values.allow_global_redeem}
                        onChange={formik.handleChange}
                        onBlur={formik.handleBlur}
                      />
                  ...
                  <AddMorePopup
                    ...
                    setServiceListing={setServiceListing}
              ...
              {formik?.values?.companies &&
                formik?.values?.companies.map((company) => (
                  ...
                  >
                    ...
                    <p className="typography-dicount-view">
                      ({companywithservice(company.id)} of{" "}
                      {companywithservicelength(company.id)} Services Selected )
                    </p>
                  </div>
                ))}
            </div>
            ...
            >
              ...
              >
                    ...
                    <ButtonGroup style={{ border: "1px solid black" }}>
                      <Button
                        style={{
                          ...
                          textTransform: "capitalize",
                          backgroundColor:
                            formik.values.purchase_requirement === false
                              ? "black"
                              : "",
                          color:
                            formik.values.purchase_requirement === false
                      ...
                      <Button
                        style={{
                          ...
                          textTransform: "capitalize",
                          backgroundColor:
                            formik.values.purchase_requirement === true
                              ? "black"
                              : "",
                          color:
                            formik.values.purchase_requirement === true
                ...
                <div
                  className={isMobileView ? "w-100" : "w-50"}
                  style={{ marginTop: isMobileView ? "5px" : "" }}
                >
                  <p style={{ fontWeight: 500, fontSize: "16px" }}>
                    Minimum Purchase Amount{" "}
                    {formik.values.purchase_requirement === true && (
                  ...
                  <input
                    ...
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    disabled={formik.values.purchase_requirement === false}
                  />
                  {formik.touched.min_purchase_amount &&
            ...
            >
              <p style={{ fontWeight: "600", fontSize: "16px" }}>
                Feature Discount (optional)
              </p>
              <Box sx={{ display: "flex", alignItems: "center" }}>
                <Checkbox
                  size="small"
                  name="on_checkout"
                  checked={formik.values.on_checkout}
                  value={formik.values.on_checkout}
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                />
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                <Checkbox
                  name="on_home_screen"
                  checked={formik.values.on_home_screen}
                  value={formik.values.on_home_screen}
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  size="small"
                />
              ...
              >
                <label htmlFor="file-input" style={{ width: "100%" }}>
                  ...
                  >
                    ...
                    <Typography
                      variant="body2"
                      style={{ fontSize: "12px", color: "#68727D" }}
                    >
                ...
                {formik?.values?.image?.name && (
                  <span className="my-3">
                    File Name : {formik.values?.image?.name}
                  </span>
                )}
              </Box>
            </div>
